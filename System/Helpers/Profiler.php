...
class Profiler {
    ...
    /**
     * Retourne l'état d'activation du profiler
     *
     * @return boolean
     */
    ...
    /**
     * Ajoute une information sur une requête BDD
     *
     * @param array queryInfos
     * @return int
     */
    ...
    /**
     * Ajoute une information sur la lecture d'un résultat BDD
     *
     * @param array queryInfos
     * @return int
     */
    ...
    /**
     * Ajoute une marqueur de temps système
     *
     * @param string name
     */
    ...
    /**
     * Calcul le temps total de MySQL
     *
     * @return float
     */
    ...
    /**
     * Calcul le temps total du fetch
     *
     * @return float
     */
    ...
    /**
     * Calcul et affiche les durées des marqueurs de temps systèmes
     *
    ...
    /**
     * Calcul et affiche les durées des marqueurs de temps
     *
     * @param float totalTime
     * @return string
     */
    ...
    /**
     * Genère l'affiche des statistiques des requêtes BDD
     *
     * @return string
     */
    ...
    public static function getQueriesResults(){
        ...
        foreach (self::$queries as $query) {
            ...
            $return .= '<tr><td style="padding:5px;vertical-align:top;width:290px;color:#900;font-weight:normal;background-color:#ddd;">MySQL(&nbsp;' . round($query[1], 4) . '&nbsp;ms&nbsp;)' . $strFetch . '&nbsp;&nbsp;</td><td style="padding:5px;color:#000;font-weight:normal;background-color:#ddd;"><code style="background-color:#F7F7F9;border: 1px solid #E1E1E8;color: #DD1144;padding: 2px 4px;"><span style="color: #000000">' . $strQuery . '</span></code></td></tr>';
            $i++;
        }
        return $return;
    }
    ...
    /**
     * Genère l'affichage des headers
     *
     * @return string
     */

    private static function headers_output() {
        $headersOutput = '<fieldset style="border:1px solid #000;padding:6px 10px 10px 10px;margin:20px 0 20px 0;background-color:#eee;" id="ci_profiler_http_headers"><legend style="color:#000;">&nbsp;&nbsp;HTTP HEADERS&nbsp;&nbsp;(<span onclick="var s=document.getElementById(\'ci_profiler_httpheaders_table\').style;s.display=s.display==\'none\'?\'\':\'none\';this.innerHTML=this.innerHTML==\'Show\'?\'Hide\':\'Show\';" style="cursor: pointer;">Show</span>)</legend><table style="width:100%;background-color: transparent;max-width: 100%;display:none;" id="ci_profiler_httpheaders_table">' . "\n";

        foreach (array('HTTP_ACCEPT', 'HTTP_USER_AGENT', 'HTTP_CONNECTION', 'SERVER_PORT', 'SERVER_NAME', 'REMOTE_ADDR', 'SERVER_SOFTWARE', 'HTTP_ACCEPT_LANGUAGE', 'SCRIPT_NAME', 'REQUEST_METHOD',' HTTP_HOST', 'REMOTE_HOST', 'CONTENT_TYPE', 'SERVER_PROTOCOL', 'QUERY_STRING', 'HTTP_ACCEPT_ENCODING', 'HTTP_X_FORWARDED_FOR') as $header) {
            $val = isset($_SERVER[$header]) ? $_SERVER[$header] : '';
            $headersOutput .= '<tr><td style="vertical-align:top;width:50%;padding:5px;color:#900;background-color:#ddd;">' . $header . '&nbsp;&nbsp;</td><td style="width:50%;padding:5px;color:#000;background-color:#ddd;">' . $val . "</td></tr>\n";
        }
    ...
    /**
     * Genère l'affichage de la session
     *
     * @return string
     */
    ...
    /**
     * Affecte le temps de rendu de la vue
     *
     * @param float renderingTime
     */
    ...
    /**
     * Genère l'affichage du profiler
     *
     * @param float totalTime
     * @param float controllerTime
     */
    ...
    public static function displayProfiler($totalTime, $controllerTime){
        if (self::enabled() && $GLOBALS['conf']['environment'] != 'prod') {
            ...
            echo '<div style="clear:both;background-color:#fff;padding:10px;" id="codeigniter_profiler"><fieldset style="border:1px solid #900;padding:6px 10px 10px 10px;margin:20px 0 20px 0;background-color:#eee;" id="ci_profiler_benchmarks"><legend style="color:#900;">&nbsp;&nbsp;BENCHMARKS&nbsp;&nbsp;</legend>
                ...
                <tr><td style="padding:5px;width:30%;color:#000;font-weight:bold;background-color:#ddd;">Controller Execution Time ( ' . Request::getController() . '#' . Request::getAction() . ' )&nbsp;&nbsp;</td><td style="padding:5px;width:70%;color:#900;font-weight:normal;background-color:#ddd;">' . $controllerTime . ' ms<div style="width:600px;background-color:#EEEEEE;height:20px;border-radius: 3px 3px 3px 3px;"><div style="height:20px;border-radius: 3px 3px 3px 3px;background-image: linear-gradient(45deg, rgba(255, 255, 255, 0.15) 25%, transparent 25%, transparent 50%, rgba(255, 255, 255, 0.15) 50%, rgba(255, 255, 255, 0.15) 75%, transparent 75%, transparent);color:#ffffff;text-align:center;text-shadow: 0px 0px 5px #000000;filter:dropshadow(color=#000000, offx=0, offy=0);background-size: 40px 40px;width:' . $controllerPourcent . '%;background-color:' . $controllerColor . '">' . round($controllerPourcent) . '%</div></div></td></tr>
                ...
                <tr><td style="padding:5px;width:30%;color:#000;font-weight:bold;background-color:#ddd;">Total Execution Time&nbsp;&nbsp;</td><td style="padding:5px;width:70%;color:#900;font-weight:normal;background-color:#ddd;">' . $totalTime . ' ms</td></tr></tbody></table></fieldset>
                ...
                <legend style="color:#000;">&nbsp;&nbsp;CONTROLLER DETAILS&nbsp;&nbsp;(<span onclick="var s=document.getElementById(\'ci_profiler_ctrl_table\').style;s.display=s.display==\'none\'?\'\':\'none\';this.innerHTML=this.innerHTML==\'Show\'?\'Hide\':\'Show\';" style="cursor: pointer;">Show</span>)</legend>
                    <div id="ci_profiler_ctrl_table" style="display:none;">' . $controlleurOutput . '</div></fieldset></div>';
        }
    }
