...
class EasyRecord
{
    ...
    public function __construct($fromQuery = false)
    {
        ...
        if (!static::getConfig('scopes'))
            $this->scopes();
        if (!static::getConfig('defaultScopes'))
            $this->defaultScopes();
        $this->errors = new ErreurManager;
    }
    ...
    public static function &getConfig($key = null)
    {
        ...
        $config = [
            ...
            'defaultScopes'         => static::$defaultScopes,
    ...
    public static function getIdentifier($lower = false)
    {
        if ($lower)
            return strtolower(static::identifiant());
    ...
    public static function getDefaultScopes()
    {
        ...
        return static::getConfig('defaultScopes');
    }
    ...
    public function __set($key, $value)
    {
        if ($key !== null) {
            ...
            if (in_array($string, static::getConfig('belongsTo')) || array_key_exists($string, static::getConfig('belongsTo'))) {
                return EasyRecordRelations::setBelongsTo($string, $value, $this);
            } else {
                if (in_array($key, static::getConfig('attrAccessor')))
                    return $this->$key = $value;
                if ($this->hasSetMutator($key)) {
                    $method = 'set'.UnderscoreToCamel($key).'Attribute';
    ...
    {
        Cache::delete('EasyRecordCache_'.SITE_ID.get_class($this).'_'.$this->getIdentifierValue().$this::cacheActivation().$this::cacheTime());
    }
    ...
    public function scope($name, $function)
    {
        if (!is_callable($function))
            throw new EasyRecordException(get_class($this).'::scope() expects paramter 2 to be a function, '.gettype($function).' given');
        $scopes =& static::getConfig('scopes');
        $scopes[$name] = $function;
    }

    public function defaultScope($name, $function)
    {
        if (!is_callable($function))
            throw new EasyRecordException(get_class($this).'::defaultScope() expects paramter 2 to be a function, '.gettype($function).' given');
        $defaultScopes =& static::getConfig('defaultScopes');
        $defaultScopes[$name] = $function;
    }
    ...
    public static function create(array $params = [], $bypassAttrAccessible = false)
    {
        ...
        if ($massAssignmentErrors)
            throw new EasyRecordException('can\'t mass-assign protected attributes: '.implode(', ', $massAssignmentErrors), 2);
        $nouvelObjet->save();
        return $nouvelObjet;
    }
    ...
    {
        if ($this->readOnly)
            throw new EasyRecordException('readOnly objects can not be saved', 3);
        ...
        execute($sqlQuery, $bindParam);
        if ($this->isNew()) {
            if ($lastId = $GLOBALS['Database']->insertId())
        ...
        Cache::delete('EasyRecordCache_'.SITE_ID.get_class($this).'_'.$this->getIdentifierValue().self::cacheActivation().self::cacheTime());
        Cache::put('EasyRecordCache_'.SITE_ID.get_class($this).'_'.$this->getIdentifierValue().self::cacheActivation().self::cacheTime(), $this, self::cacheTime());
    ...
    public function updateAttributes($attributes, $bypassAttrAccessible = false ,$save = true)
    {
        ...
        if (!is_array($attributes))
            throw new EasyRecordException(get_class($this).'::updateAttributes() expects parameter 1 to be array, '.gettype($attributes).' given');
        $config =& static::getConfig();
        ...
        if (spl_object_hash($this) == $GLOBALS['update_attributes_object_hash']) {
            ...
            if ($GLOBALS['update_attributes_mass_assignment_errors'])
                throw new EasyRecordException('can\'t mass-assign protected attributes: '.implode(', ', $GLOBALS['update_attributes_mass_assignment_errors']), 2);
    ...
    public function delete()
    {
        if ($this->getIdentifierValue()) {
            ...
            $query = implode(' ', array(
                'DELETE FROM',
                quoteIdentifiant($database).'.'.quoteIdentifiant($table),
                'WHERE',
                quoteIdentifiant(self::getIdentifier(false)),
                '= '.$this->getIdentifierValue(),
            ));
            execute($query, new BindParam);
    ...
    {
        if (!$this->relations)
            $this->relations = new EasyRecordRelations;
    ...
    private function buildUpdate()
    {
        ...
        $requete = [];
        $requete[] = 'UPDATE '.quoteIdentifiant(static::database()).'.'.quoteIdentifiant(static::table()).' SET';

        $listeChamps = [];
        foreach ($this->dirty as $field => $value) {
            if (!array_key_exists($field, $this->expr)) {
                $listeChamps[] = quoteIdentifiant($field).' = ?';
                $type_colonne = 's';
                if (array_key_exists($field, static::mapping()))
                    $type_colonne = static::mapping()[$field]['type'][0];
                $bindParam->add($type_colonne, $value);
            } else {
                $listeChamps[] = quoteIdentifiant($field).' = '.($value === null ? 'NULL' : $value);
            }
        }
        $requete[] = implode(', ', $listeChamps);
        $requete[] = 'WHERE';
        $requete[] = quoteIdentifiant(static::getIdentifier(false));
    ...
    private function buildUpdateGroup($ids)
    {
        ...
        foreach ($this->dirty as $field => $value) {
            if (!array_key_exists($field, $this->expr)) {
                ...
                $listeChamps[] = quoteIdentifiant($field).' = '.($value === '' ? 'NULL' : $value);
            }
        }
        $requete[] = implode(', ', $listeChamps);
        $requete[] = 'WHERE';
    ...
    private function buildInsert()
    {
        $bindParam = new BindParam();
        $requete[] = 'INSERT INTO';
        $requete[] = quoteIdentifiant(static::database()).'.'.quoteIdentifiant(static::table());
        $listeChamps = array_map('quoteIdentifiant', array_keys($this->dirty));
        $requete[] = '(' . implode(', ', $listeChamps) . ')';
        $requete[] = 'VALUES';
        $requete[] = '(';
        ...
        foreach ($hasManies as $key => $hasMany) {
            if (is_array($hasMany) && array_key_exists('dependent', $hasMany) && !array_key_exists('through', $hasMany)) {
                ...
                if ($hasMany['dependent'] == 'destroy') {
                    ...
                    $GLOBALS['Database']->query('DELETE FROM '.$objetName::database().'.'.$objetName::table().' WHERE '.$cleEtrangere.' = "'.$this->getIdentifierValue().'"');
                    return true;
        ...
        foreach (static::getConfig('hasOne') as $key => $hasOne) {
            if (is_array($hasOne) && array_key_exists('dependent', $hasOne)) {
                ...
                if ($hasOne['dependent'] == 'destroy') {
                    ...
                    $GLOBALS['Database']->query('DELETE FROM '.$key::table().' WHERE '.$cleEtrangere.' = "'.$this->getIdentifierValue().'"');
    ...
    private function deleteHasAndBelongsToMany()
    {
        ...
        foreach (static::getConfig('hasAndBelongsToMany') as $key => $hasAndBelongsToMany) {
            ...
            $GLOBALS['Database']->query('DELETE FROM '.$concatTableDb.' WHERE '.$concatCleEtrangere.' = "'.$this->getIdentifierValue().'"');
        }
    }
    ...
    {
        return new EasyRecordQuery($this);
    }

    protected function hasSetMutator($key)
    {
        return method_exists($this, 'set'.UnderscoreToCamel($key).'Attribute');
    }
    ...
    protected function defaultScopes() {
    }

    protected static function mappingInit()
    {
        $className = get_called_class();
        if (!$mapping = Cache::get('EasyRecordMapping'.SITE_ID.$className)) {
            $modelTab = explode('\\',$className);
            $model = end($modelTab);
            $mapping       = \Spyc::YAMLLoad(MAPPING.DIRECTORY_SEPARATOR.$model.'.yml')[$model];
            Cache::put('EasyRecordMapping'.SITE_ID.$className, $mapping, 60);
        }
        $config =& static::getConfig();
        if (!$config['mapping']) {
            $config['database']      = $mapping['database'];
            $config['table']         = $mapping['table'];
            $config['identifiant']   = $mapping['identifiant'];
    ...
    private function setProperty($key, $value = null, $expr = false)
    {
        if (!is_array($value)) {
            if (!array_key_exists($key, static::mapping())) {
                ...
                throw new EasyRecordException('Try to assign to a non defined attribute or accessor ('.$key.')');
            }
            if (static::mapping()[$key]['type'] == 'integer') {
                if (!is_numeric($value) && $value !== null) {
                    debug(debug_backtrace());
                    throw new EasyRecordException('Try to assign a non-numeric value ('.$value.') to a numeric field ('.$key.')');
                } else {
                    if (!is_null($value)) {
                        $value = $value + 0;
                    }
